...
'use strict';
var propertyParser = require('propertyParser');
var rParts = /^([^,]+)(?:,([^,]+))?(?:,(.+))?/;
...
return {
    load : function(name, req, onLoad, config){
        if (config.isBuild) {
            ...
            req(['async!'+ (window.document.location.protocol === 'https:'? 'https' : 'http') +'://www.google.com/jsapi'], function(){
                window.google.load(data.moduleName, data.version, settings);
            });
        }
    }
};
